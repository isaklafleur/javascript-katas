...
function roundIt(n) {
    ...
    var stringLength = n.toString().length;
    var leftSide = parseInt(n.toString().substring(0,dot));
    var rightSide = parseInt(n.toString().substring(dot+1,stringLength));

    if(leftSide.toString().length === rightSide.toString().length) return Math.round(n);
    if(leftSide < rightSide) return Math.ceil(n);
    if(leftSide > rightSide) return Math.floor(n);
}
roundIt(3.45)
//roundIt(34.5)
//roundIt(34.56)
