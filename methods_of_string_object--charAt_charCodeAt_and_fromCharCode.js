...
function topSecret(str){
  ...
  for (var i=0, len = encoded.length; i < len; i++) {
      var sym = encoded[i].charCodeAt();
      if( !((sym < 123 && sym > 96) || (sym > 64 && sym < 91)) ) continue;
      encoded[i] = String.fromCharCode(encoded[i].charCodeAt()-3);
      
      encoded[i] = (encoded[i] == ">" ? "X" :
                    encoded[i] == "?" ? "Y" :
                    encoded[i] == "@" ? "Z" :
                    encoded[i] == "^" ? "x" :
                    encoded[i] == "_" ? "y" :
  ...
  var decoded = encoded.join('');
  return decoded;
}
topSecret("wklv lv dq hadpsoh")
